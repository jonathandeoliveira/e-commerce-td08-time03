class ProductPrice < ApplicationRecord
  ...
  validate :date_is_future, :start_date_differs_from_end_date, :end_date_is_future_from_start_date, :price_unique_within_dates
  ...
  def price_unique_within_dates
    if self.price.present? && self.start_date.present? && self.end_date.present?
      prices = ProductPrice.where(product_model: self.product_model_id)
      prices.each do | price |
        if (self.start_date >= price.start_date && self.start_date <= price.end_date)
          self.errors.add(:price, ' não pode ser cadastrado, pois está em incluso em intervalo de datas já existentes')
        end
      end
    end    
  end
